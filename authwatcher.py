...
msgres = ['error: channel_setup_fwd_listener: cannot listen to port\: (?P<lport_cannotlisten>[\d]+)'
          ...
          ,'PAM (?P<num_authfails2>[\d]+) more authentication failures; logname= uid=0 euid=0 tty=ssh ruser= rhost=(?P<raddr_authfails2>[\w\d\.\-]+) (| user=(?P<user_authfails2>[\w]+))'
          ...
          ,'(error: |)Received disconnect from (?P<rhost_disconnect>[\d\.]+): (?P<errcode_disconnect>\d+): (?P<disconnect_reason>[\w]*) \[preauth\]'
          ,'(error: |)Received disconnect from (?P<rhost_userdisconnect>[\d\.]+): 11: (?P<disconnect2_reason>[\w]*)'
          ,'(error: |)Received disconnect from (?P<rhost_userdisconnect2>[\d\.]+): (?P<disconnect3_code>\d+): (?P<disconnect3_reason>[\w ]*) \[preauth\]'
          ...
          ,'Did not receive identification string from (?P<raddr_didnotreceive>[\d\w\-\.]+)'
